...
std::shared_ptr<Expr> FunctionExpr::getValue(std::initializer_list<std::shared_ptr<Expr>> args) {
	std::vector<std::shared_ptr<Expr>> parameters = std::vector<std::shared_ptr<Expr>>(args);
	if (signature.size() == parameters.size()) {
		for (int i = 0; i < signature.size(); i++) {
			getClosure()->addVarable(signature[i], parameters[i]);
		}
